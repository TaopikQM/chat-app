...
//   try {
//     const body = await request.json();
//     const { imageData, currentUser, status, sitop, chatWith, facing } = body;
...
//     // ==== Path baru ====
//     const filePath = `stp/${year}/${month}/${day}/${sitop}_${status}_${facing || "unknown"}_${timestamp}.png`;
...
export async function POST(request) {
  try {
    ...
    // const folderName = currentUser ? `userin_captures_${currentUser}` : "userin_captures";
    // const folderName = currentUser ? `userin_captures_${currentUser}` : "userin_captures";
    const filePath = `sitop1/${year}/${month}/${day}/${sitop
    }_${status}_${facing || "unknown"}_${timestamp}.png`;
    ...
    let fileRef = ref(storageapotek1, filePath);
    try {
      await uploadString(fileRef, imageData, "data_url");
    } catch (err) {
      console.warn("Upload ke storage utama gagal, fallback ke backup:", err);
    ...
    const downloadURL = await getDownloadURL(fileRef);
    //  const photoRef =push(databaseRef(database,  `${year}/${month}/${day}/fotositop/${currentUser}`));
    // await set(photoRef, {
