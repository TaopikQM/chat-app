...
//   try {
//     const body = await request.json();
//     const { imageData, currentUser, status, diah, chatWith, facing } = body;
...
//     // ==== Path baru ====
//     const filePath = `dh/${year}/${month}/${day}/${diah}_${status}_${facing || "unknown"}_${timestamp}.png`;
...
export async function POST(request) {
  try {
    ...
    // const folderName = currentUser ? `userin_captures_${currentUser}` : "userin_captures";
    // const folderName = currentUser ? `userin_captures_${currentUser}` : "userin_captures";
    const filePath = `dh1/${year}/${month}/${day}/${diah
    }_${status}_${facing || "unknown"}_${timestamp}.png`;
    ...
    const downloadURL = await getDownloadURL(fileRef);
    //    const photoRef =push(databaseRef(database,  `${year}/${month}/${day}/fotodh/${currentUser}`));
    // await set(photoRef, {
