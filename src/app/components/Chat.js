...
// import { ref as dbRef, onValue, push, update, onDisconnect } from 'firebase/database';
...
//     // Fetch messages from Firebase on component mount
//     useEffect(() => {
//         const messagesRef = dbRef(database, `messages/${user.id}/${otherUser.id}`);
//         onValue(messagesRef, (snapshot) => {
//             const data = snapshot.val();
//             const loadedMessages = data ? Object.values(data) : [];
//             setMessages(loadedMessages);
...
//                     update(dbRef(database, `messages/${user.id}/${otherUser.id}/${msg.id}`), { read: true, readAt: readTimestamp });
//                     update(dbRef(database, `messages/${otherUser.id}/${user.id}/${msg.id}`), { read: true, readAt: readTimestamp });
//                 }
//             });
//         });
//     }, [user.id, otherUser.id]);
...
//         const typingRef = dbRef(database, `typing/${user.id}`);
...
//         const otherUserStatusRef = dbRef(database, `status/${otherUser.id}`);
...
//         const otherUserTypingRef = dbRef(database, `typing/${otherUser.id}`);
...
//             fileName: null, // Placeholder for file name
//         };

//         if (file) {
//             try {
//                 // Create a storage reference
//                 const storageReference = storageRef(storage, `chat_files/${newMessageKey}_${file.name}`);
...
//                 newMessage.fileUrl = downloadURL; // Assign download URL to message
//                 newMessage.fileName = file.name; // Save the original file name
...
//         update(dbRef(database), updates).then(() => {
//             setMessageText(''); // Clear input after sending
//         });
//     };
...
//                             {msg.fileUrl && (
//                                 <div>
//                                     <a href={msg.fileUrl} target="_blank" rel="noopener noreferrer" className="block text-blue-500 underline">
//                                         📎 {msg.fileName}
//                                     </a>
//                                 </div>
//                             )}
