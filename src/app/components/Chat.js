...
const Chat = ({ user }) => {
  ...
  const [loading, setLoading] = useState(false); // Untuk fitur "loading" (laseen)
    ...
    onValue(messagesRef, (snapshot) => {
      ...
      loadedMessages.forEach((msg) => {
        if (!msg.read && msg.sender !== user.id) {
          update(ref(database, `messages/${user.id}/${otherUser.id}/${msg.id}`), {
            read: true,
          });
          update(ref(database, `messages/${otherUser.id}/${user.id}/${msg.id}`), {
            read: true,
          });
        }
      });
  ...
  const sendMessage = async () => {
    ...
    setUploading(true);
    setLoading(true); // Mulai "loading" (laseen)
    ...
    <div className="flex flex-col h-screen bg-gray-100">
          ...
          >
            <div
              className={`inline-block p-2 rounded-lg ${
                msg.sender === user.id ? "bg-blue-500 text-white" : "bg-gray-300"
              }`}
            >
              {msg.text}
              {renderMediaLinks(msg.files)}
            </div>
      ...
      <div className="flex items-center p-4 border-t border-gray-300">
        ...
        <button
          ...
          disabled={uploading}
        >
          {loading ? "Sending..." : "Send"} {/* Tampilkan status loading */}
        </button>
      </div>
    </div>
  );
