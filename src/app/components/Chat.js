...
const Chat = ({ user }) => {
    ...
    useEffect(() => {
        ...
        update(userStatusRef, { online: true, lastSeen: Date.now() }).catch(error => console.error("Error updating status:", error));

        // Set user to offline and record last seen when they disconnect
        onDisconnect(userStatusRef).update({ online: false, lastSeen: Date.now() }).catch(error => console.error("Error setting offline:", error));

        // Update typing status when the user types
        if (messageText.trim() || files.length > 0) {
            update(typingRef, { typing: true }).catch(error => console.error("Error updating typing status:", error));
        } else {
            update(typingRef, { typing: false }).catch(error => console.error("Error updating typing status:", error));
        }

        // Listen for the other user's online status
        const otherUserStatusRef = ref(database, `status/${otherUser.id}`);
        const unsubscribeOtherUserStatus = onValue(otherUserStatusRef, (snapshot) => {
        ...
        const unsubscribeOtherUserTyping = onValue(otherUserTypingRef, (snapshot) => {
            const data = snapshot.val();
            setIsOtherUserTyping(data?.typing || false);
        }, (error) => {
            console.error("Error fetching other user typing status:", error);
        });

        // Cleanup on component unmount
        return () => {
            unsubscribeOtherUserStatus();
            unsubscribeOtherUserTyping();
            update(typingRef, { typing: false }).catch(error => console.error("Error stopping typing:", error));
        };
    ...
    const sendMessage = async () => {
        ...
        const newMessage = {
            ...
            id: newMessageKey,
            files: fileURLs.filter(url => url !== null), // Store uploaded file URLs, filtering out any null values
        };
                ...
                <div className="flex items-center">
                    {otherUserStatus?.online ? (
                        ...
                        <span className="text-gray-500">
                            Last seen: {new Date(otherUserStatus?.lastSeen).toLocaleTimeString()}
                        </span>
                    )}
