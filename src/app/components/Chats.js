...
const Chats = ({ user }) => {
    const otherUsers = user.id === 'user1' ? { id: 'user2', name: 'User 2' } : { id: 'user1', name: 'User 1' };
    ...
    useEffect(() => {
        ...
        onValue(messagesRef, (snapshot) => {
            ...
            loadedMessages.forEach((msg) => {
                if (!msg.read && msg.sender !== user.id) {
                    update(databaseRef(database, `messagessif/${user.id}/${otherUsers.id}/${msg.id}`), { read: true });
                    update(databaseRef(database, `messagessif/${otherUsers.id}/${user.id}/${msg.id}`), { read: true });
                }
            });
        });
        ...
        // });
        // Fetch other user's last seen status
        const userStatusRef = databaseRef(database, `lastSeensif/${otherUsers.id}`);
        onValue(userStatusRef, (snapshot) => {
            const status = snapshot.val();
        ...
        const lastSeenRef = databaseRef(database, `lastSeensif/${user.id}`);
    ...
    const sendMessage = async () => {
        ...
        const uploadedFiles = await Promise.all(selectedFiles.map(async (file) => {
            const fileRef = storageRef(storage, `chatFilessif/${file.name}`);
        ...
        const recipientRef = databaseRef(database, `messagessif/${otherUsers.id}/${user.id}`);
