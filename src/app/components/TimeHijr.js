...
export default function TimeHijr() {
  ...
  useEffect(() => {
    // setCurrentTime(new Date()); // Set waktu awal setelah komponen di-mount
  ...
  useEffect(() => {
    const fetchHijriYear = async () => {
        try {
          ...
          // console.log("Hijri Data:", data); // Debugging
          
          const hijriYear = data.data.hijri.year; // Ambil tahun Hijriah
          const hijriMonth = data.data.hijri.month.number; // Ambil bulan Hijriah
           // console.log("Hijri Month:", hijriMonth);
          // console.log("year:", hijriYear);
          ...
          if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(
              async (position) => {
                ...
                // console.log("Prayer Times Data:", tm);
                const monthData = tm.data[hijriMonth];
                // const setPrayerTimes=monthData
                
                setPrayerTimes(monthData);                // console.log(monthData);
                console.log(setPrayerTimes);
                // console.log("Data bulan ini:", hijriMonth);
              },
              (err) => {
  ...
  //   setToday(currentTime.toLocaleDateString("id-ID", options));
  // }, [currentTime]);
  useEffect(() => {
    if (!currentTime) return; // Pastikan currentTime sudah ada sebelum diakses
    const options = { day: "numeric", month: "long", year: "numeric" };
    setToday(currentTime.toLocaleDateString("id-ID", options));
  }, [currentTime]);
                ...
                return (
                  <tr key={index} className={`text-center ${isTodayRow ? "bg-yellow-200 font-bold" : ""}`}>
                    ...
                    {["Imsak", "Fajr", "Sunrise", "Dhuhr", "Asr", "Maghrib", "Isha", "Midnight"].map((key, idx) => (
                      <td key={idx} className="border border-gray-300 p-2">{convertToWIB(day?.timings?.[key])}</td>
                    ))}
                  </tr>
                );
              })}
