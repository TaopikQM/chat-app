...
const ProvinsiPulauForm = () => {
  ...
  const handlePulauChange = (event) => {
    ...
    const selectedPulauData = pulauProvinsiData.find((item) => item.name === selectedPulau);
    setPulau(selectedPulau);
    setProvinsiList(selectedPulauData ? selectedPulauData.provinsi : []);
  };

  // Simpan Pulau & Provinsi ke Firebase dengan Urutan
  const handleSimpanProvinsi = async () => {
    ...
    try {
      // Cari urutan pulau berdasarkan nama
      const selectedPulauData = pulauProvinsiData.find((item) => item.name === pulau);
      if (!selectedPulauData) {
        alert("Pulau tidak ditemukan dalam data!");
        return;
      }

      const { urutan } = selectedPulauData;
      const pulauRef = databaseRef(database, `pulau/${pulau}`);

      // Simpan data Pulau beserta urutan
      await set(pulauRef, {
        name_pulau: pulau,
        urutan_pulau: urutan,
      });

      // Simpan data Provinsi dalam Pulau
      const provinsiRef = databaseRef(database, `pulau/${pulau}/provinsi`);
      provinsiList.forEach(async (provinsi, index) => {
        const newProvRef = push(provinsiRef);
        await set(newProvRef, {
          id: newProvRef.key,
          name_provinsi: provinsi,
          urutan_provinsi: index + 1,
          createdAt: new Date().toISOString(),
        });
      });

      alert(`Semua provinsi di ${pulau} berhasil disimpan dengan urutan!`);
    } catch (error) {
