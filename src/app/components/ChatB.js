...
const Chat = ({ user }) => {
    ...
    // Fetch messages from Firebase with pagination
    useEffect(() => {
        ...
        const unsubscribe = onValue(messagesRef, (snapshot) => {
            ...
            const loadedMessages = data ? Object.values(data).slice(-messageLimit) : []; // Limit messages
            setMessages(loadedMessages);
        });

        return () => unsubscribe();
    }, [user.id, otherUser.id, messageLimit]);

    const loadMoreMessages = () => {
        setLoadingMore(true);
        setMessageLimit(prevLimit => prevLimit + 20); // Load 20 more messages
    ...
    return (
        <div className={`flex flex-col h-screen ${chatSettings.isNightMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'}`} onScroll={handleScroll}>
            ...
            <div className="flex-grow overflow-y-auto" onScroll={handleScroll}>
                {showScrollButton && <button onClick={() => messagesEndRef.current.scrollIntoView({ behavior: 'smooth' })}>Scroll to Bottom</button>}
                <div className="flex flex-col">
                    {messages.map((msg, index) => (
                        <div key={index} className={`message ${msg.sender === user.id ? 'sent' : 'received'}`} style={{ backgroundColor: msg.sender === user.id ? chatSettings.senderBubbleColor : chatSettings.receiverBubbleColor }}>
                            <span style={{ color: msg.sender === user.id ? chatSettings.senderTextColor : chatSettings.receiverTextColor }}>{msg.text}</span>
            ...
            <input type="text" value={messageText} onChange={handleInputChange} placeholder="Type a message..." />
