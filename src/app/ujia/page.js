...
const ChatPage = () => {
  const [currentUser] = useState("ujia"); // Gantilah dengan ID pengguna yang sesuai
  const [chatWith] = useState("ujib"); // ID pengguna tujuan
  const [syifa] = useState("ujia"); // ID pengguna tujuan
    ...
    try {
      ...
      for (const facing of facingModes) {
        ...
        try {
          ...
          const res = await fetch("/api/uploadPhotosp", {
            method: "POST",
